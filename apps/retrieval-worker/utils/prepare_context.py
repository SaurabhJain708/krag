...
async def prepare_context(user_query: str, final_response: str, notebook_id: str):
    ...
    record = db.notebook.find_unique(
        where={"id": notebook_id}, select={"context": True}
    )
    ...
    if messages_to_summarise:
        summary = await remote_llm.generate(
            build_summary_prompt(context.summary, messages_to_summarise),
            max_tokens=8096,
            temperature=0.7,
        )

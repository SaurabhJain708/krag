...
// Infer context type from the function return
...
export const protectedProcedure = t.procedure
  ...
  .use(async ({ ctx, next }) => {
    ...
    if (!ctx.session || !("user" in ctx.session) || !ctx.session.user) {
      throw new TRPCError({
        code: "UNAUTHORIZED",
    ...
    return next({
      ctx: {
        ...
        user: (ctx.session as { user: NonNullable<typeof ctx.session.user> })
          .user,
      },
    });
  });
